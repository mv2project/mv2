...
public class WebSpaceManagerImpl implements WebSpaceManager {
	 ...
	 * @param context The context with the connection to the database.
	 * @param certManager The certificate manager to use.
	 */
	public WebSpaceManagerImpl(SQLContext context, CertificateManager certManager) {
	...
	public WebSpace createWebSpace(String identifier, Certificate cert) {
		try{
			PreparedStatement ps = context.getConnection().prepareStatement(CREATE_WEB_SPACE);
	...
	@Override
	public boolean canCreate(String identifier, String location, String state,
			String country, String organization, String organizationUnit,
			PublicKey key) {
		return true;
	}
	...
	public WebSpace getWebSpace(String identifier) throws NoSuchElementException {
		try{
			PreparedStatement ps = context.getConnection().prepareStatement(LOAD_WEB_SPACE);
			ps.setString(1, identifier);
			ResultSet rs = ps.executeQuery();
			if(rs.next()){
				Certificate cert = certManager.load(new BigInteger(rs.getBytes("certificate")));
				return new WebSpaceImpl(rs.getString("identifier"), cert);
			}
			throw new NoSuchElementException("No web space found for the given identifier (" + identifier + ").");
