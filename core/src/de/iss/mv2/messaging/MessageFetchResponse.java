...
public class MessageFetchResponse extends MV2Message {
	 ...
	 * @param message The encoded message to set.
	 * @throws IllegalArgumentException Is thrown if the given data is {@code null} or empty.
	 */
	public void setContentMessage(byte[] message) throws IllegalArgumentException {
		if(message == null || message.length == 0) throw new IllegalArgumentException("The message must not be null or empty."); 
		setMessageField(new MessageField(DEF_MESSAGE_FIELD.CONTENT_BINARY, message), true);
	}
	 ...
	 * @throws IllegalArgumentException Is thrown if the given timestamp is {@code null}.
	 */
	public void setTimestamp(Date timestamp) throws IllegalArgumentException{
		if(timestamp == null) throw new IllegalArgumentException("The timestamp must not be null.");
		setMessageField(new MessageField(DEF_MESSAGE_FIELD.TIMESTAMP, BinaryTools.toByteArray(timestamp.getTime())), true);
	}
	
	/**
	 * Returns the timestamp specifying when the containing message was received.
	 * @return The timestamp specifying when the containing message was received.
	 */
